 ...
 *
 * THIS FILE IS THE SOURCE OF TRUTH. The copy running in Google is a deployment
 * of it. Edit here, commit, then paste into the Apps Script editor - not the
 * other way round. The script used to live only in Google, with no history, and
 * that is precisely how it drifted out of step with the sheet without anyone
 * noticing (see "The dead-column bug" below).
 *
 ...
 *
 * ---------------------------------------------------------------------------
 * THE DEAD-COLUMN BUG, and why ensureHeaders_ looks the way it does now.
 *
 * The sheet was created with headers 'Student Name' and 'App Name'. The schema
 * later standardised on 'Student' and 'App', with LEGACY_RENAMES to migrate.
 * But the rename only fired when the target was ABSENT:
 *
 *     if (to && headers.indexOf(to) === -1) { headers[i] = to; }
 *
 * Once any earlier run had appended 'Student' at the far right, that condition
 * was false forever. So 'Student Name' was never renamed. It froze as a dead
 * column holding the OLD rows' names, while every new row wrote its name into
 * the appended 'Student' column instead.
 *
 * The visible symptom: the student name column simply stopped filling in from
 * the day the schema changed, while every other column kept working. Nothing
 * errored. Rows up to 12 Jul had names; rows from 16 Jul looked anonymous.
 *
 * Two changes fix it for good:
 *   1. ensureHeaders_ no longer silently skips a rename it cannot perform. It
 *      records the collision so it is visible instead of invisible.
 *   2. repairLegacyColumns() consolidates any duplicate that already exists -
 *      merging values into the canonical column, then removing the dead one.
 *      Run it once from the Edutrack menu; it is idempotent.
 * ---------------------------------------------------------------------------
 */
...
function setupAll() {
  var repaired = repairLegacyColumns_(tab_(SESSIONS_TAB));   // before headers are ensured
  ...
  setupDashboard();
  setupStudentDetail();
  var msg = 'Setup complete.' + (repaired.length ? '\n\nColumns repaired:\n- ' + repaired.join('\n- ') : '');
...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('Edutrack')
    ...
    .addItem('Repair legacy columns', 'repairLegacyColumns')
...
// Read the header row, trimmed, with trailing blanks dropped.
...
/**
 * Merge one column's values into another, FILLING BLANKS ONLY.
 *
 * Never overwrites a value that is already in the target: the target holds the
 * live data and the source holds history, so the target always wins a conflict.
 * Returns how many cells were filled.
 */
function mergeColumnValues_(sheet, fromIdx, toIdx) {
  var last = sheet.getLastRow();
  if (last < 2) return 0;
  var n = last - 1;
  var src = sheet.getRange(2, fromIdx + 1, n, 1).getValues();
  var dst = sheet.getRange(2, toIdx + 1, n, 1).getValues();
  var moved = 0;
  for (var i = 0; i < n; i++) {
    var s = src[i][0] === null || src[i][0] === undefined ? '' : String(src[i][0]).trim();
    var d = dst[i][0] === null || dst[i][0] === undefined ? '' : String(dst[i][0]).trim();
    if (s && !d) { dst[i][0] = src[i][0]; moved++; }
  }
  if (moved) sheet.getRange(2, toIdx + 1, n, 1).setValues(dst);
  return moved;
}

/**
 * Consolidate legacy duplicate columns. Idempotent - safe to run any time.
 *
 * For each LEGACY_RENAMES pair where BOTH the old and new header exist, the old
 * one is a dead column: new rows have been writing to the new one. This merges
 * the old values into the new column (blanks only, so nothing live is lost) and
 * then deletes the dead column.
 *
 * Deletion is the one destructive step here, and it only ever runs against a
 * column whose values have just been copied somewhere else.
 *
 * Returns a list of human-readable descriptions of what it did.
 */
function repairLegacyColumns_(sheet) {
  var headers = readHeaders_(sheet);
  var report = [];
  var deletions = [];

  Object.keys(LEGACY_RENAMES).forEach(function (legacy) {
    var canonical = LEGACY_RENAMES[legacy];
    var from = headers.indexOf(legacy);
    if (from === -1) return;                       // no legacy column: nothing to do
    var to = headers.indexOf(canonical);

    if (to === -1) {
      headers[from] = canonical;                   // target free: a plain rename
      report.push('"' + legacy + '" renamed to "' + canonical + '"');
      return;
    }
    if (to === from) return;

    var moved = mergeColumnValues_(sheet, from, to);
    report.push('"' + legacy + '" merged into "' + canonical + '" (' +
                moved + ' value(s) recovered), dead column removed');
    deletions.push(from);
    headers[from] = null;                          // mark; removed from the array below
  });

  if (deletions.length) {
    // Right to left, so earlier indices stay valid as columns shift left.
    deletions.sort(function (a, b) { return b - a; })
             .forEach(function (i) { sheet.deleteColumn(i + 1); });
    headers = headers.filter(function (h) { return h !== null; });
  }
...
// Menu wrapper: same repair, with a report.
function repairLegacyColumns() {
  var report = repairLegacyColumns_(tab_(SESSIONS_TAB));
  var msg = report.length
    ? 'Repaired:\n- ' + report.join('\n- ')
    : 'Nothing to repair - no duplicate legacy columns found.';
...
function ensureHeaders_(sheet, wanted, renames) {
  ...
  if (renames) {
    for (var i = 0; i < headers.length; i++) {
      ...
      if (!to) continue;
      var at = headers.indexOf(to);
      if (at === -1) {
        headers[i] = to;
        changed = true;
      } else if (at !== i) {
        // BOTH headers exist. The old code silently skipped here, which is what
        // stranded 'Student Name' as a dead column for weeks. Do not repair it
        // inside a POST - deleting a column while a student is mid-sync is not a
        // trade worth making - but do not hide it either.
        collisions.push(headers[i] + ' -> ' + to);
      }
    }
  }
  ...
  if (collisions.length) {
    // Visible in Executions, and flagged on the header cell so it is noticed by
    // someone looking at the sheet rather than only by someone reading logs.
    Logger.log('Duplicate legacy columns, run "Repair legacy columns": ' + collisions.join(', '));
    try {
      sheet.getRange(1, 1).setNote(
        'Duplicate legacy columns detected: ' + collisions.join(', ') +
        '\nRun Edutrack > Repair legacy columns to consolidate them.');
    } catch (e) {}
  }
  return headers;
}

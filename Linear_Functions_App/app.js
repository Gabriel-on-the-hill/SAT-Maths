(function () {
    const APP_ID = 'Linear_Functions_App';
    const state = {
        ...
        isExamMode: false,
        isHardMode: false,
        smartMode: false,        // wrong-first prioritization
        isRetrySession: false,   // true when drilling missed questions

        timeRemaining: 900,
        timerInterval: null,
        reviewIndex: 0,
        questionStartTime: 0,
        sessionStartTime: 0,    // set on each renderSlide() for time tracking
    ...
    function saveProgress(isFinal = false) {
        ...
        const appName = "Linear_Functions_App";
        // If final, list all topics they covered
        let topicName = state.playlist[state.currentTopicIdx]?.title || "Multi-Topic Session";

        // If they did multiple topics, label it explicitly
        if (state.currentTopicIdx > 0 && isFinal) {
            const allTitles = state.playlist.slice(0, state.currentTopicIdx + 1).map(t => t.title).join(" + ");
            topicName = allTitles;
        }

        // Get the Section Name (e.g., "Section A: Guided Practice") based on the moduleKey
        let sectionName = state.currentModule; // fallback
        const moduleBtn = document.querySelector(`.module-btn[data-module="${state.currentModule}"] h3`);
        if (moduleBtn) {
            sectionName = moduleBtn.textContent.trim();
        }

        const fullTopicContext = `${topicName} | ${sectionName}`;
    ...
    function startPlaylist(moduleKey) {
        ...
        // Smart Mode: flatten all topics' questions and prioritize wrong → unseen → mastered.
        // Falls back to original topic-by-topic flow if MathProgress unavailable.
...
// Scale the clock to the session length (~90s per question = realistic SAT
    ...
    function startWeakArea() {
        ...
        // Build cross-module pool, deduped by id
        const sourcePlaylist = window.PLAYLIST || [];
        const seen = new Set();
        const pool = [];
        sourcePlaylist.forEach(topic => {
            const qsByMod = topic.questions || {};
            Object.values(qsByMod).forEach(qs => {
        ...
        state.isExamMode = dom.examModeToggle.checked;
        state.isHardMode = false;       // weak-area uses all difficulties
        state.smartMode = false;         // already prioritised by build function
        state.isRetrySession = false;
        state.currentModule = 'homework'; // record as homework source (no feedback during quiz)
        state.currentTopicIdx = 0;
        state.allQuestions = [];
        state.userAnswers = [];
    ...
    function _buildCrossModulePool() {
        ...
        const pool = [];
        sourcePlaylist.forEach(topic => {
            const qsByMod = (topic && topic.questions) || {};
            Object.values(qsByMod).forEach(qs => {
                if (!Array.isArray(qs)) return;
                qs.forEach(q => {
                    if (q && q.id && !seen.has(q.id)) {
    ...
    function _runPersonalisedSession(opts) {
        // opts: { selected, moduleKey, playlistMeta }
    ...
    // Returns a small HTML callout describing the archetype, trap, and strategy
    // for a question — but only if at least one of those fields is authored on
    // the question object. Empty when no fields are present (graceful no-op).
    ...
    function finishPlaylist() {
        ...
        state.allQuestions.forEach((q, idx) => {
            try {
                ...
                gradable++;
                const userA = state.userAnswers[idx];
                const isCorrect = (q.type === 'grid-in')
        ...
        // Surface retry-missed button only if there's something to drill
        if (dom.retryMissedBtn && dom.retryMissedCount) {
            if (missed.length > 0) {
                ...
                dom._missedQueue = missed; // stash for the click handler
            } else {
    ...
    function startRetryMissed() {
        ...
        // Reset session state for a fresh run over just the missed questions

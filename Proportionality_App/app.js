(function () {
    const APP_ID = 'Proportionality_App';
    const state = {
    ...
    function saveProgress(isFinal = false) {
        ...
        const appName = "Proportionality_App";
    ...
    function loadTopic(index) {
        ...
        let finalModuleKey = moduleKey;
        if (state.isHardMode && topic.questions[moduleKey + "_hard"]) {
            finalModuleKey = moduleKey + "_hard";
        }
        state.currentQuestions = topic.questions[finalModuleKey] || [];
    ...
    function finishPlaylist() {
        ...
        state.allQuestions.forEach((q, idx) => {
            try {
                ...
                // Defensive: a single bad question must never break scoring.
                ungraded++;
            }
        });

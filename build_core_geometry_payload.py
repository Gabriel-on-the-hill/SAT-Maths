...
def build_payload():
    # Load both JSONs
    with open('Core_Geometry_Area_Volume/snapshot_questions.json', 'r') as f:
        data1 = json.load(f)
    with open('Core_Geometry_Lines_Angles/snapshot_questions.json', 'r') as f:
        data2 = json.load(f)
    
    all_qs = data1 + data2
    
    # Clean paths (make sure they are just assets/filename.png)
    for q in all_qs:
        q['question_image'] = 'assets/' + os.path.basename(q['question_image'])
        q['skill'] = "Core Geometry"
    ...
    playlist = {
        ...
        "exam": get_set(4, 7, 4), # Slightly larger for exam
        "additional": easy_pool + medium_pool + hard_pool # All leftovers
    }

    # Format as JS
    output = f"const PLAYLIST = {json.dumps(playlist, indent=4)};\nwindow.PLAYLIST = PLAYLIST;"
    
    with open('Core_Geometry_App/data/questions.js', 'w') as f:
        f.write(output)
